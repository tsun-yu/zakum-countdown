<template>
  <div class="horntail-wrapper">
    <header class="page-header">
      <h1 class="page-title">技能計時器</h1>
...
const STORAGE_KEY = "horntail-view-timers-v1";

// Built-in「左手消技」timer.
// - Default repeats every 90 seconds.
// - Warns 10 seconds before finishing (unlike Buff's 5s).
const BUILTIN_SKILL = Object.freeze({
  id: "horntail-left-dispel",
  name: "左手消技",
  durationSec: 90,
  warnAt: 10,
  speechText: "注意左消",
...
// State ─────────────────────────────────────────────────────
const timers = ref([{ ...BUILTIN_SKILL }]);
...
function loadFromStorage() {
  try {
    ...
    const storedBuiltin = data.find((x) => x?.id === BUILTIN_SKILL.id);
    const custom = data
      .filter((x) => x && x.id !== BUILTIN_SKILL.id && !x.isBuiltin)
      .map((x) => ({
    ...
    const builtinDuration =
      ...
      BUILTIN_SKILL.durationSec;

    timers.value = [
      { ...BUILTIN_SKILL, durationSec: builtinDuration },
      ...custom,
    ];
  } catch (_) {
    /* ignore corrupt data */
